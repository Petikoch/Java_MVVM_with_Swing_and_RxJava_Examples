...
class Example_11_View extends StrictThreadingJFrame implements IView<Example_11_ViewModel> {

    private final StrictThreadingJCheckBox editCheckBox;
    private final StrictThreadingJLabel customerLabel;
    private final Example_11_View_AddressPanel addressPanel;
    private final Example_11_View_NotesPanel notesPanel;
    private final StrictThreadingJButton saveButton;

    @Override
    public void bind(final Example_11_ViewModel viewModel) {
        bindSwingView(editCheckBox).toViewModel(viewModel.v2vm_vm2v_edit);
        bindViewModel(viewModel.v2vm_vm2v_edit).toAction(editCheckBox::setSelected);
        bindViewModelString(viewModel.vm2v_customer).toSwingViewLabel(customerLabel);
        bindViewModelBoolean(viewModel.vm2v_saveButtonEnabled).toSwingViewEnabledPropertyOf(saveButton);
        bindSwingView(saveButton).toViewModel(viewModel.v2vm_saveButtonClick);

        addressPanel.bind(viewModel.addressViewModel);
        notesPanel.bind(viewModel.notesViewModel);
    }

    public Example_11_View() {
        super();
        ...
        addressPanel = new Example_11_View_AddressPanel();
        ...
        notesPanel = new Example_11_View_NotesPanel();
        GridBagConstraints gbc_notesPanel = new GridBagConstraints();
        gbc_notesPanel.gridwidth = 2;
        gbc_notesPanel.insets = new Insets(0, 0, 5, 5);
        gbc_notesPanel.fill = GridBagConstraints.BOTH;
        gbc_notesPanel.gridx = 0;
        gbc_notesPanel.gridy = 2;
        getContentPane().add(notesPanel, gbc_notesPanel);

        saveButton = new StrictThreadingJButton("Save");
